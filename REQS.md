...
* **2.5 (litmus tests)**: Members are *not* literally `str` objects (`type(HttpMethod.GET) is not str`) and will not be identical to a string object.
...
* **2.5 (litmus)**: `type(HttpMethod.GET) is not str`, and identity checks against `"GET"` won’t pass.
...
* **2.5 (litmus)**: This can satisfy your “simplicity” litmus tests because the values literally are string objects.
